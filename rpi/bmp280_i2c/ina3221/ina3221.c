...
#include <errno.h>
#include <string.h>
#include "include/ina3221_internal.h"
#include "include/ina3221_regs.h"
#include "ina3221.h"
#include <stdio.h>
...
uint16_t ntohs_generic(uint16_t netshort) {
    uint16_t hostshort = ((netshort & 0xFF00) >> 8) | // High byte shifted to low byte
                         ((netshort & 0x00FF) << 8);  // Low byte shifted to high byte
    return hostshort;
}
...
static int _read_reg(const ina3221_t *dev, uint8_t reg, uint16_t *out)
{
 ...
 //   status = i2c_read_blocking(dev->params.i2c, dev->params.addr, &tmp, 1, true);
    status = i2c_read_blocking(dev->params.i2c, dev->params.addr, out, INA3221_REG_LEN, false);
    *out = (*out >> 8) | (*out << 8);
...
static int _write_reg(const ina3221_t *dev, uint8_t reg, uint16_t in)
{
   // in = ((in & 0x00FF) << 8) | ((in & 0xFF00) >> 8); 
    ...
    status = i2c_write_blocking(dev->params.i2c, dev->params.addr, buf, 3, false);
...
#if MODULE_INA3221_ALERTS_ENABLE
