...
void read_bus_v(ina3221_t *dev){
 ...
 //   status = i2c_read_blocking(dev->params.i2c, dev->params.addr, &tmp, 1, true);
    status = i2c_read_blocking(dev->params.i2c, dev->params.addr, &out, 2, false);
    ch1_bus = (out >> 8) | (out << 8);
...
int main() {
   ...
   // scan_i2c_bus(i2c_default, 5000);
    ...
    // configure INA3221
    ina3221_t ina3221;
    ina3221_params_t ina3221_params;
    ina3221_params.i2c = i2c2;
    ina3221_params.addr = 64;
    ina3221_params.config = INA3221_DEFCONFIG;
    ina3221_params.rshunt_mohm[0] = 10;
    ina3221_params.rshunt_mohm[1] = 20;
    ina3221_params.rshunt_mohm[2] = 30;
    sleep_ms(1000);

  //  test_ina(&ina3221_params);
    int status = ina3221_init(&ina3221,  &ina3221_params);
    printf("INA init status: %d\n", status);
...
*/
        gpio_put(PICO_DEFAULT_LED_PIN, 1);
        ina3221_read_shunt_uv(&ina3221, bus_voltages, &flags);
        printf("Channels:  %dmV, %dmV, %dmV\n", bus_voltages[0], bus_voltages[1], bus_voltages[2]);

        ina3221_read_shunt_uv(&ina3221, shunt_uv, &flags);
        printf("Shunts:  %duV, %duV, %duV\n", shunt_uv[0], shunt_uv[1], shunt_uv[2]);

       // printf("flags status: %d\n", ina3221_read_flags(&ina3221, &flags));
        read_bus_v(&ina3221);
        sleep_ms(1000);
        gpio_put(PICO_DEFAULT_LED_PIN, 0);
        sleep_ms(500);
