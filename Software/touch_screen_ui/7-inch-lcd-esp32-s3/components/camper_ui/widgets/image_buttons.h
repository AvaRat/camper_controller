...
static void create_lights_brightness_popup(PowerSwitch_t *pwr_switch){
   ...
   // lv_msgbox_add_text(mbox1, "This is a message box with two buttons.");
    ...
    lv_obj_t *obj = lv_msgbox_get_header(mbox);
    lv_obj_set_height(obj, 50);
    obj = lv_msgbox_get_title(mbox);
    
    lv_obj_set_style_text_font(obj, font_large, LV_PART_MAIN);
...
//    lv_obj_set_style_pad_all(btn_obj, 5, LV_PART_MAIN);
    ...
    lv_obj_set_style_border_width(parent, 10, LV_PART_MAIN);


    lv_obj_t *btn_label = lv_label_create(parent);
    lv_label_set_text(btn_label, power_switch->btn_text);
    lv_obj_set_style_text_font(btn_label, font_large, LV_PART_MAIN);
    lv_obj_set_style_pad_all(btn_label, 5, LV_PART_MAIN);
