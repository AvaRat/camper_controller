...
static void set_x_pos_cb(lv_anim_t *anim, int32_t val)
{
    lv_obj_set_x((lv_obj_t *) anim->var, val);
    lv_anim_t * anim_2 = (lv_anim_t*)anim->user_data;

    //set also y pos to go back after seccond animation to start y coordinate
    lv_obj_set_pos((lv_obj_t *) anim->var, val, anim_2->start_value);
}


static void set_y_pos_cb(lv_anim_t *anim, int32_t val)
{
    lv_obj_set_y((lv_obj_t *) anim->var, val);
}

static void set_y_pos_pct_cb(lv_anim_t *anim, int32_t val)
{
    lv_obj_set_y((lv_obj_t *) anim->var, lv_pct(val));
}


static void ay_cmpl_cb(lv_anim_t *anim)
{
    printf("ay completed\n");
}
